...
export default function RegisterPage() {
  ...
  const strength = pw.length > 12 ? 4 : pw.length > 8 ? 3 : pw.length > 4 ? 2 : pw.length > 0 ? 1 : 0
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '80px 24px' }}>
      <div style={{ width: '100%', maxWidth: 400 }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          ...
          <h1 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 800, fontSize: 30, letterSpacing: '-0.03em', marginBottom: 6 }}>Create account</h1>
          <p style={{ color: '#9090a8', fontFamily: 'DM Sans, sans-serif' }}>Start your typing journey today</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ background: '#131318', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 18, padding: 28 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              <label style={{ display: 'block', fontFamily: 'JetBrains Mono, monospace', fontSize: 12, color: '#9090a8', marginBottom: 8 }}>Username</label>
              <input type="text" value={username} onChange={e => setUsername(e.target.value)} placeholder="speedyfingers"
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 4, marginTop: 8, alignItems: 'center' }}>
                {[1, 2, 3, 4].map(i => (
                  <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: strength >= i ? sColors[strength] : 'rgba(255,255,255,0.06)', transition: 'background 0.2s' }} />
                ))}
                {strength > 0 && <span style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: sColors[strength], marginLeft: 4, whiteSpace: 'nowrap' }}>{sLabels[strength]}</span>}
              </div>
            </div>
        ...
        <p style={{ textAlign: 'center', marginTop: 20, fontSize: 14, color: '#55556a', fontFamily: 'DM Sans, sans-serif' }}>
          Have an account?{' '}
          <Link href="/login" style={{ color: '#e8ff57', textDecoration: 'none', fontWeight: 600 }}>Sign in</Link>
        </p>
      </div>
    </div>
  )
