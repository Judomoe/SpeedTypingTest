  ...
  return (
    <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100 }}>
      <div style={{
        ...
        background: 'rgba(12,12,16,0.85)', backdropFilter: 'blur(20px)',
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {NAV.map(n => (
            <Link key={n.href} to={n.href} style={{
              ...
              color: pathname === n.href ? '#e8ff57' : '#9090a8',
              background: pathname === n.href ? 'rgba(232,255,87,0.08)' : 'transparent',
            }}>{n.label}</Link>
          ))}
        </div>
          ...
          {user ? (
            <div style={{ position: 'relative' }}>
              <button onClick={() => setMenuOpen(o => !o)} style={{
                ...
                <div style={{
                  width: 26, height: 26, borderRadius: '50%', background: '#e8ff57',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
              ...
              {menuOpen && (
                <div style={{
                  ...
                  {[{ href: '/profile', label: '👤  Profile' }, { href: '/settings', label: '⚙️  Settings' }].map(item => (
                    <Link key={item.href} to={item.href} onClick={() => setMenuOpen(false)} style={{
                      display: 'block', padding: '9px 14px', borderRadius: 8,
                      fontFamily: 'DM Sans, sans-serif', fontSize: 14, color: '#9090a8', textDecoration: 'none',
                    }}>{item.label}</Link>
                  ))}
            ...
            <>
              <Link to="/login" style={{
                ...
                color: '#9090a8', border: '1px solid rgba(255,255,255,0.08)',
              }}>sign in</Link>
              <Link to="/register" style={{
                padding: '7px 16px', borderRadius: 8, textDecoration: 'none',
                fontFamily: 'DM Sans, sans-serif', fontSize: 13, fontWeight: 600,
                color: '#0c0c10', background: '#e8ff57',
              }}>get started</Link>
            </>
          )}
        </div>
