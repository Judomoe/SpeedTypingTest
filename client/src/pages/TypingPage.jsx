...
const WORD_BANKS = {
  ...
  french: [
    "la dactylographie est l art de taper sur un clavier sans regarder les touches en utilisant la mémoire musculaire",
    "la pratique quotidienne est essentielle pour améliorer votre vitesse de frappe et votre précision sur le clavier",
    "chaque grande compétence nécessite du temps et de la persévérance avant de devenir une seconde nature pour vous",
  ],
  spanish: [
    "la mecanografía es el arte de escribir con el teclado sin mirar las teclas usando la memoria muscular",
    "la práctica diaria es esencial para mejorar tu velocidad de escritura y tu precisión en el teclado",
  ...
  german: [
    ...
    "tägliche Übung ist unerlässlich um Ihre Tippgeschwindigkeit und Genauigkeit auf der Tastatur zu verbessern",
  ],
  italian: [
    "la dattilografia è l arte di scrivere sulla tastiera senza guardare i tasti usando la memoria muscolare",
    "la pratica quotidiana è essenziale per migliorare la velocità di battitura e la precisione sulla tastiera",
  ],
}
...
const LESSON_TEXTS = {
  ...
  'num-3':    '10% 25% 50% 75% 100% $9.99 $19.99 $99.00 €50 £30 ¥1000 #42 @speed +100 -50',
}
...
export default function TypingPage() {
  ...
  useEffect(() => {
    if (started && !done) {
      timerRef.current = setInterval(() => {
        ...
        // In zen mode there's no timer countdown
  ...
  // Navigate to results when done (not zen)
  useEffect(() => {
    if (!done || !started || mode === 'zen') return
    ...
    const result = {
      ...
      dur: elapsed, mode, wpmData: wpmPerSecRef.current, rawData: rawPerSecRef.current, errData: errAtSecRef.current,
    }
  ...
  function onInput(e) {
    ...
    // ZEN mode: free typing, no limit, no correct/wrong tracking beyond display
    if (mode === 'zen') {
  ...
  // Click anywhere on the typing area → focus and clear blur
    ...
    >
      <div style={{ width: '100%', maxWidth: 820, padding: '40px 24px' }}>
        ...
        <div
          ...
          onClick={handleAreaClick}
        >
          {/* Click-to-start overlay — disappears on click anywhere */}
          {isBlurred && (
            <div style={{
              position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center',
              borderRadius: 16, background: 'rgba(12,12,16,0.6)', backdropFilter: 'blur(4px)', zIndex: 5, cursor: 'text',
            }}>
          ...
          {mode === 'zen' ? (
            // ZEN: show textarea for free typing
              ...
              {text.split('').map((ch, i) => {
                ...
                return (
                  <span key={i} style={{ color, background: bg, borderRadius: 2, position: 'relative' }}>
                    {i === typed.length && (
                      <span style={{
                        position: 'absolute', left: settings.caretStyle === 'under' ? 0 : 0,
                        top: settings.caretStyle === 'block' ? 0 : 0,
                        width: settings.caretStyle === 'block' ? '100%' : settings.caretStyle === 'under' ? '100%' : 2,
                        height: settings.caretStyle === 'under' ? 2 : settings.caretStyle === 'block' ? '100%' : '100%',
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, marginTop: 24 }}>
          ...
          {mode === 'zen' && started && (
            <button onClick={e => { e.stopPropagation(); setDone(true) }} style={{
