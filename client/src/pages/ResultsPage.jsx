...
// Lesson counts per tab — must match CoursesPage
const LESSON_COUNTS = { basic: 10, prog: 5, num: 3 }
...
export default function ResultsPage() {
  ...
  const [nextUrl, setNextUrl]     = useState('/typing')
  const [retryUrl, setRetryUrl]   = useState('/typing')
  const [isLesson, setIsLesson]   = useState(false)
  const [nextLabel, setNextLabel] = useState('Next test')
  ...
  useEffect(() => {
    try {
      ...
      if (lessonId && lessonTab) {
        ...
        const doneKey = `${tabKey}-${lessonId}`
        const completed = JSON.parse(localStorage.getItem('tc_completed') || '{}')
        completed[doneKey] = true
        localStorage.setItem('tc_completed', JSON.stringify(completed))

        // Retry = same lesson
        setRetryUrl(`/typing?lesson=${lessonId}&tab=${lessonTab}`)

        // Next lesson
        const maxLessons = LESSON_COUNTS[tabKey] || 10
        if (lessonId < maxLessons) {
          setNextUrl(`/typing?lesson=${lessonId + 1}&tab=${lessonTab}`)
          setNextLabel(`Lesson ${lessonId + 1} →`)
        } else {
          // Last lesson in this tab — go back to courses
          setNextUrl('/courses')
          setNextLabel('Back to courses 🎉')
          setIsLastLesson(true)
        }
      } else {
        setNextUrl('/typing')
        setRetryUrl('/typing')
        setNextLabel('Next test')
      }
    } catch {}
  ...
  // Tab+Enter shortcut — goes to next (lesson or typing)
  useEffect(() => {
    let tabDown = false
    const down = (e) => {
      if (e.key === 'Tab') { e.preventDefault(); tabDown = true }
      if (e.key === 'Enter' && tabDown) navigate(nextUrl)
    }
  ...
  const { wpm, rawWpm, acc, consistency, correct, incorrect, extra, missed, dur, mode, wpmData, rawData, errData, lessonId, lessonTab } = result
  ...
  const tabKey = lessonTab
    ? (lessonTab.startsWith('prog') ? 'prog' : lessonTab.startsWith('num') ? 'num' : 'basic')
    : null
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'flex-start', justifyContent:'center', padding:'90px 24px 60px' }}>
      <div style={{ width:'100%', maxWidth:780 }}>
        ...
        {/* Lesson badge */}
        {isLesson && (
          <div style={{ ...fadeUp(0), textAlign:'center', marginBottom:20 }}>
            <span style={{ padding:'6px 18px', borderRadius:100, border:'1px solid rgba(87,255,216,0.25)', background:'rgba(87,255,216,0.06)', fontFamily:'JetBrains Mono, monospace', fontSize:13, color:'#57ffd8' }}>
              ✓ Lesson {lessonId} complete — {tabKey}
            </span>
            {isLastLesson && (
              <p style={{ fontFamily:'Outfit, sans-serif', fontWeight:700, fontSize:18, color:'#e8ff57', marginTop:12 }}>
                🎉 You finished all {tabKey} lessons!
              </p>
            )}
          </div>
        )}
        ...
        <div style={{ ...fadeUp(0.08), background:'#131318', border:'1px solid rgba(255,255,255,0.06)', borderRadius:18, padding:'22px 32px', marginBottom:10 }}>
          <div style={{ display:'grid', gridTemplateColumns:'140px 1px 80px 1px minmax(0,1fr) 1px 90px 1px 80px', gap:0, alignItems:'start' }}>
            <div style={{ paddingRight:24 }}>
              ...
              <div style={{ fontFamily:'JetBrains Mono, monospace', fontSize:14, color:'#e8ff57', lineHeight:2 }}>
                {isLesson ? `lesson ${lessonId}` : `time ${dur}`}<br />{isLesson ? tabKey : mode}
              </div>
            </div>
        ...
        <div style={{ ...fadeUp(0.16), background:'#131318', border:'1px solid rgba(255,255,255,0.06)', borderRadius:18, padding:'18px 24px', display:'flex', alignItems:'center', justifyContent:'space-between', gap:20, flexWrap:'wrap' }}>
          <div style={{ display:'flex', alignItems:'center', gap:16 }}>
            ...
            <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
              {/* Primary: next lesson OR next test */}
              <Link to={nextUrl} style={{
                display:'flex', alignItems:'center', gap:6,
                padding:'10px 18px', borderRadius:10,
                border:'1px solid rgba(232,255,87,0.3)', background:'rgba(232,255,87,0.08)',
                fontFamily:'DM Sans, sans-serif', fontWeight:600, fontSize:14,
                color:'#e8ff57', textDecoration:'none',
              }}>{nextLabel}</Link>

              {/* Retry — only shown for lessons */}
              {isLesson && (
                <Link to={retryUrl} title="retry this lesson" style={{
                  width:44, height:44, display:'flex', alignItems:'center', justifyContent:'center',
                  borderRadius:10, border:'1px solid rgba(255,255,255,0.07)',
                  background:'rgba(255,255,255,0.02)',
                  fontFamily:'JetBrains Mono, monospace', fontSize:18,
                  color:'#55556a', textDecoration:'none',
                }} title="Retry this lesson">↺</Link>
              )}

              {/* Courses link — always shown for lessons */}
              {isLesson && (
                <Link to="/courses" title="back to courses" style={{
                  width:44, height:44, display:'flex', alignItems:'center', justifyContent:'center',
                  borderRadius:10, border:'1px solid rgba(255,255,255,0.07)',
                  background:'rgba(255,255,255,0.02)',
                  fontFamily:'JetBrains Mono, monospace', fontSize:18,
              ...
              {/* Standard buttons for free typing */}
              {!isLesson && (
                <>
                  <Link to="/typing" title="restart" style={{ width:44, height:44, display:'flex', alignItems:'center', justifyContent:'center', borderRadius:10, border:'1px solid rgba(255,255,255,0.07)', background:'rgba(255,255,255,0.02)', fontFamily:'JetBrains Mono, monospace', fontSize:18, color:'#55556a', textDecoration:'none' }}>↺</Link>
                  <Link to="/leaderboard" title="leaderboard" style={{ width:44, height:44, display:'flex', alignItems:'center', justifyContent:'center', borderRadius:10, border:'1px solid rgba(255,255,255,0.07)', background:'rgba(255,255,255,0.02)', fontFamily:'JetBrains Mono, monospace', fontSize:18, color:'#55556a', textDecoration:'none' }}>≡</Link>
                  <Link to="/profile" title="history" style={{ width:44, height:44, display:'flex', alignItems:'center', justifyContent:'center', borderRadius:10, border:'1px solid rgba(255,255,255,0.07)', background:'rgba(255,255,255,0.02)', fontFamily:'JetBrains Mono, monospace', fontSize:18, color:'#55556a', textDecoration:'none' }}>⏮</Link>
                </>
              )}
            </div>
          </div>
        ...
        <p style={{ ...fadeUp(0.24), textAlign:'center', marginTop:24, fontFamily:'JetBrains Mono, monospace', fontSize:12, color:'#3a3a50' }}>
          ...
          {' '}to {isLesson ? 'go to next lesson' : 'restart'}
        </p>
      </div>
    </div>
  )
